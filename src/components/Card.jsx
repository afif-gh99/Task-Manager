const toneClasses = {
  ...
  completed: {
    ...
    ambientColor: "var(--color-status-done-bg)",
  },
};
...
const Card = ({ icon, title, val, tone }) => {
  ...
  return (
    <div
      style={{ "--ambient-color": classes.ambientColor }}
      className={`${classes.surface} card-ambient animate-fade-up relative isolate flex items-center gap-3 overflow-hidden rounded-[24px] border border-[var(--color-border-strong)] px-4 py-4 shadow-[var(--color-shadow-soft)] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(49,74,143,0.12)] sm:gap-4 sm:rounded-[28px] sm:px-5 sm:py-5 md:rounded-[30px] md:px-6 md:py-6`}
    >
      <span
        aria-hidden="true"
        className="card-ambient-wave absolute inset-0"
      />
      <span
        aria-hidden="true"
      ...
      <div className="relative z-10 flex h-12 w-12 shrink-0 items-center justify-center rounded-[20px] bg-white/80 shadow-[0_12px_30px_rgba(37,46,89,0.08)] backdrop-blur-sm sm:h-14 sm:w-14 sm:rounded-[22px] md:h-[4.5rem] md:w-[4.5rem] md:rounded-3xl">
        <div
      ...
      <div className="relative z-10 flex flex-col justify-center leading-none">
