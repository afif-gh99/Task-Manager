...
const TaskForm = ({
  ...
  return (
    <div className="animate-fade-up mt-8">
      <div className="mb-8 flex flex-wrap items-center gap-3 text-(--color-text-secondary)">
        <Link
          to={backTo}
          className="inline-flex items-center gap-2 text-sm font-semibold transition hover:text-(--color-primary) md:text-base"
        >
          <span className="text-xl leading-none">&larr;</span>
          {backLabel}
        </Link>
        {badgeLabel && (
          <div className="inline-flex items-center rounded-full bg-(--color-primary-soft) px-4 py-2 text-xs font-bold text-(--color-primary) md:text-sm">
            New {badgeLabel}
          </div>
        )}
      </div>
      ...
      <div className="mb-8 max-w-5xl">
        <h1 className="text-2xl font-extrabold tracking-[-0.03em] text-(--color-text-primary) md:text-3xl lg:text-4xl">
          {title}
        </h1>
        <p className="mt-3 max-w-3xl text-base text-(--color-text-secondary) md:text-lg">
          {subtitle}
        </p>
      </div>

      <div className="rounded-[34px] border border-(--color-border-strong) bg-(--color-surface-elevated) p-6 shadow-(--color-shadow-soft) backdrop-blur-sm transition-all duration-300 md:p-7">
        <form onSubmit={onSubmit} className="space-y-7">
          <label className="flex flex-col gap-2">
            <span className="text-xl font-extrabold tracking-tight text-(--color-text-primary) md:text-2xl">
              Task title
            </span>
            <input
              id="title"
          ...
          <label className="flex flex-col gap-3">
            ...
            <textarea
              ...
              required
              rows={7}
              className="rounded-[28px] border border-(--color-border-strong) bg-(--color-surface-soft) px-5 py-4 text-base text-(--color-text-primary) outline-none transition duration-200 placeholder:text-(--color-text-muted) focus:border-(--color-primary) focus:bg-white md:text-lg"
            />
          </label>
          ...
          <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-end">
            <Link
              to="/dashboard"
              className="inline-flex h-12 items-center justify-center px-2 text-base font-bold text-(--color-text-secondary) transition duration-200 hover:text-(--color-primary) md:h-14 md:text-lg"
            >
              Cancel
            </Link>
            <button
